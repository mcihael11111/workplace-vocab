...
export default function App() {
  ...
  const { completedTerms, toggleComplete, streakDays, longestStreak, todayCount } = useProgress(user, { onMilestone: handleMilestone, onNudge: handleNudge });
  ...
  // toast can be a string or { message, action, onAction }
  const showToast = (msg) => {
    setToast(typeof msg === "string" ? { message: msg } : msg);
    setTimeout(() => setToast(null), 4000);
  };

  // Detect category completion and fire a toast on the completing toggle
  const prevCompletedRef = useRef(null);
  useEffect(() => {
    if (!user || prevCompletedRef.current === null) {
    ...
    for (const cat of CATEGORIES) {
      ...
      const doneNow   = words.every(w => completedTerms.has(w.term));
      const doneBefore = words.every(w => prev.has(w.term));
  ...
  // Show success toast when Stripe redirects back with ?session_id=
  useEffect(() => {
  ...
  // Sign out, reset to home view, show confirmation toast
